...
指定した複数の値に合致するデータを取得する。WHERE 句内でサブクエリーを使う場合、サブクエリーは複数の値を返すので、WHERE 句の条件式として『=』が使えない。"そのサブクエリーの条件に合致していること"を意味して『IN』を使用する。

### ANY について

指定したいずれかの値と一致するかどうかを検索条件とする場合に使用する。IN が and で ANY が or 的なこと…？

### EXISTS について

サブクエリーの結果が存在するか確認するために使う。

## 1 つの SQL で同じテーブルを結合する

## 自己結合

同じテーブル同士を結合すること。フォロー・フォロワーのアソシエーションの考え方に近い。

## 相関サブクエリー

メインクエリーの値をサブクエリー内で使用するサブクエリー。サブクエリーの WHERE 句に、メインクエリーの値を使用する。
